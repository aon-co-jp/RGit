...
> ⚠️ **正直な開示**: Gitea/GitBucketが持つIssue・Pull Request・Wiki・
> Webhookは**まだ実装していません**。それ以外(Web UI・認証・アクセス
> 制御)は実装済みです。実装済みなのは以下の通りです。
...
- **WASM製Web UI**(`/ui/`): リポジトリ一覧・README閲覧・ログイン・管理パネル(申請一覧・アカウント管理・グループ管理・リポジトリ別アクセス設定)
